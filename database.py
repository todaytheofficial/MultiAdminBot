...
# MongoDB URI - получаем из переменных окружения
...
class MongoDB:
    """Синглтон для MongoDB подключения"""
...
class GroupDatabase:
    ...
    def update_rating(self, user_id: int, rating_change: int, is_win: bool):
        ...
        # Не даём рейтингу уйти в минус
        self._col("users").update_one(
            {"user_id": user_id, "rating": {"$lt": 0}},
            {"$set": {"rating": 0}}
        )
    ...
    def get_my_listings(self, user_id: int) -> list:
        cursor = self._col("market").find(
            {"seller_id": user_id, "sold": False}
    ...
    def get_top_by_mults(self, limit: int = 10) -> list:
        cursor = self._col("users").find(
            {"mults": {"$gt": 0}}
        ).sort("mults", DESCENDING).limit(limit)
    ...
    def get_warnings(self, user_id: int) -> int:
        ...
        self._col("warnings").delete_many({
            "user_id": user_id,
    ...
    def remove_one_warning(self, user_id: int):
        warn = self._col("warnings").find_one(
            {"user_id": user_id},
            sort=[("created_at", DESCENDING)]
        )
        if warn:
    ...
    def get_expired_punishments(self) -> list:
        now = datetime.now().isoformat()
        cursor = self._col("punishments").find({
            "is_active": True,
            "expires_at": {"$ne": None, "$lt": now}
        })
        return [{
            "id": str(d["_id"]),
            "user_id": d["user_id"],
            "punishment_type": d["punishment_type"],
    ...
    def deactivate_punishment(self, punishment_id):
        from bson import ObjectId
        try:
            self._col("punishments").update_one(
                {"_id": ObjectId(punishment_id)},
                {"$set": {"is_active": False}}
            )
        except:
            pass
    ...
    def use_promo_code(self, user_id: int, promo_code: str):
        try:
            self._col("used_promos").insert_one({
                "user_id": user_id,
    ...
    def create_event(self, name: str, description: str, event_type: str, start_date: str, end_date: str, rewards: dict, created_by: int):
    ...
    def join_event(self, event_id, user_id: int):
        from bson import ObjectId
        try:
    ...
    def add_event_score(self, event_id, user_id: int, score: int):
        from bson import ObjectId
        try:
            self._col("event_participants").update_one(
                {"event_id": ObjectId(event_id), "user_id": user_id},
    ...
    def get_event_leaderboard(self, event_id, limit: int = 10) -> list:
        from bson import ObjectId
        try:
            cursor = self._col("event_participants").find(
    ...
    def get_user_event_score(self, event_id, user_id: int) -> int:
        from bson import ObjectId
        try:
            doc = self._col("event_participants").find_one({
                "event_id": ObjectId(event_id),
                "user_id": user_id
            })
    ...
    def get_marriage(self, user_id: int):
        doc = self._col("marriages").find_one({
    ...
    def create_marriage(self, user1_id: int, user2_id: int):
        self._col("marriages").insert_one({
            "user1_id": user1_id,
            "user2_id": user2_id,
            "married_at": datetime.now().isoformat()
        })

    def delete_marriage(self, user_id: int):
        self._col("marriages").delete_one({
            "$or": [{"user1_id": user_id}, {"user2_id": user_id}]
        })

    def get_all_marriages(self) -> list:
        cursor = self._col("marriages").find().sort("married_at", ASCENDING)
        return [{
            "id": str(d["_id"]),
            "user1_id": d["user1_id"],
            "user2_id": d["user2_id"],
            "married_at": d["married_at"]
        } for d in cursor]

    def create_proposal(self, proposer_id: int, target_id: int):
        result = self._col("proposals").insert_one({
    ...
    def get_pending_proposal(self, proposer_id: int, target_id: int):
        doc = self._col("proposals").find_one({
            "proposer_id": proposer_id,
            "target_id": target_id,
            "status": "pending"
        })
    ...
    def update_proposal_status(self, proposal_id, status: str):
        from bson import ObjectId
        try:
            self._col("proposals").update_one(
    ...
    def get_user_rank(self, user_id: int):
        ...
        if doc:
            return {
                "user_id": user_id,
                "rank_level": doc.get("rank_level", 0),
    ...
    def record_message(self, user_id: int):
        today = date.today().isoformat()
        try:
            self._col("message_stats").update_one(
                {"user_id": user_id},
                {"$inc": {"message_count": 1}, "$set": {"last_message_date": today}},
                upsert=True
            )
            self._col("message_history").update_one(
    ...
    def get_top_messages_today(self, limit: int = 10) -> list:
        today = date.today().isoformat()
        cursor = self._col("message_history").find(
            {"date": today}
        ).sort("count", DESCENDING).limit(limit)
        return [{"user_id": d["user_id"], "count": d["count"]} for d in cursor]

    def get_top_messages_month(self, limit: int = 10) -> list:
    ...
    def get_top_messages_all_time(self, limit: int = 10) -> list:
...
class DatabaseManager:
    ...
    def get_all_group_dbs(cls) -> List[GroupDatabase]:
        # В MongoDB сложнее получить все группы, но можно из глобальной БД
...
class GlobalDatabase:
    ...
    def _ensure_indexes(self):
        try:
            ...
            self.db.spin_boosts.create_index("user_id", unique=True)
        except:
            pass
    ...
    def find_by_username(self, username: str):
        doc = self.db.users_global.find_one(
    ...
    def get_spin_boost(self, user_id: int) -> float:
        ...
        doc = self.db.spin_boosts.find_one({"user_id": user_id})
        return doc["multiplier"] if doc else 1.0

    def remove_spin_boost(self, user_id: int):
        self.db.spin_boosts.delete_one({"user_id": user_id})
