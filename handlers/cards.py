...
@router.message(Command("aispin"))
async def cmd_aispin(msg: Message):
    if not is_group(msg):
        return await msg.reply(f"{EMOJI['cross']} Только в группах!")
    ...
    tickets = db.get_spin_tickets(uid)
    if tickets < 2:
        return await msg.reply(f"🤖 <b>AI спин = 2 билета!</b>\n🎟️ У тебя: {tickets}", parse_mode="HTML")

    if not db.use_spin_ticket(uid) or not db.use_spin_ticket(uid):
        return await msg.reply(f"{EMOJI['cross']} Ошибка!")

    thinking = await msg.reply("🤖 <i>Думаю...</i>", parse_mode="HTML")
    ...
    name = msg.from_user.first_name or "Чел"
    power = card["attack"] + card["defense"]
    mood = "крутая карта, похвали с подколом" if card["rarity"] in ["mega", "special", "mythic", "legendary"] else "отстой, назови лохом"

    try:
        resp = await asyncio.get_event_loop().run_in_executor(None, lambda: g4f.ChatCompletion.create(
            model=g4f.models.default,
            messages=[{"role": "user", "content": f"Токсичный бот. Игрок {name} получил {card['name']} (сила {power}). {mood}. 2 предложения, дерзко, без markdown."}]
        ))
        comment = resp.strip().replace("**", "").replace("*", "")[:300] if resp else f"{name}, ты выбил {card['name']}... нуб 🤡"
    except:
        comment = f"{name}, {card['name']} (💪{power})... мой мозг сломался 🤡"
    ...
    pity = db.get_pity_counters(uid)
    caption = SPIN_HEADERS.get(card["rarity"], "🤖 <b>AI!</b>\n\n") + format_card(card)
    caption += f"\n\n🪙 <b>+{coins}</b>\n🎟️ Билетов: <b>{db.get_spin_tickets(uid)}</b>"
    caption += f"\n💸 <i>-2 билета</i>"
    if boost > 1:
        caption += f"\n🍀 Удача: x{boost}"
